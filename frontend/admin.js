...
async function doLogin() {
    ...
    try {
        const res = await fetch(API_BASE + '/admin/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, password: pass }) });
...
async function doRegister() {
    ...
    if (!email || !pass) { errEl.textContent = 'Please enter email and password.'; errEl.style.display = 'block'; return; }
    try {
        const res = await fetch(API_BASE + '/admin/register', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: email.split('@')[0], email, password: pass, role: 'super_admin' }) });
        const data = await res.json();
        if (data.success) { setToken(data.token); currentAdmin = data.admin; sessionStorage.setItem('admin_data', JSON.stringify(data.admin)); showDashboard(); }
...
function applyRolePermissions() {
    ...
    const hideForDelivery = ['products', 'customers', 'inventory', 'cms', 'messages'];
    const hideForManager = ['cms'];
...
async function checkAuth() {
    ...
    try {
        ...
        const data = await res.json();
        if (data.success) { currentAdmin = data.admin; sessionStorage.setItem('admin_data', JSON.stringify(data.admin)); showDashboard(); }
        else { clearToken(); document.getElementById('login-screen').classList.remove('hidden'); }
    } catch { clearToken(); document.getElementById('login-screen').classList.remove('hidden'); }
}
...
function statusBadge(s) {
    const m = { confirmed: 'badge-green', pending: 'badge-amber', out_for_delivery: 'badge-blue', delivered: 'badge-blue', cancelled: 'badge-red', active: 'badge-green', paused: 'badge-amber', unread: 'badge-amber', read: 'badge-gray', replied: 'badge-blue', out_of_stock: 'badge-red' };
    return `<span class="badge ${m[s] || 'badge-gray'}">${(s || '').replace(/_/g, ' ')}</span>`;
}
function payBadge(p) { const icons = { upi: '📱', card: '💳', netbanking: '🏦', cod: '💵' }; return `<span style="font-size:13px;">${icons[p] || '💰'} ${(p || '').toUpperCase()}</span>`; }
...
function setApiStatus(ok) { const el = document.getElementById('api-status'); const txt = document.getElementById('api-status-text'); el.className = 'api-status ' + (ok ? 'ok' : 'err'); txt.textContent = ok ? 'Connected' : 'Offline'; }

// ══════════════════════════════════════════════════════
//  API
// ══════════════════════════════════════════════════════
async function apiFetch(path) { const res = await fetch(API_BASE + path, { headers: authHeaders() }); if (!res.ok) throw new Error(`HTTP ${res.status}`); return res.json(); }
...
async function loadAll() {
    ...
    try {
        ...
        setApiStatus(true);
        const [ordRes, subRes, msgRes, prodRes] = await Promise.all([
            apiFetch('/orders?limit=500'), apiFetch('/subscriptions?limit=500'),
            apiFetch('/messages?limit=500'), apiFetch('/products')
        ]);
        allOrders = ordRes.orders || []; allSubs = subRes.subscriptions || [];
        allMsgs = msgRes.messages || []; allProducts = prodRes.products || [];
        setBadge('nb-orders', allOrders.filter(o => o.status === 'pending' || o.status === 'confirmed').length);
        setBadge('nb-subs', allSubs.filter(s => s.status === 'active').length);
        setBadge('nb-msgs', allMsgs.filter(m => m.status === 'unread').length);
        renderOverview();
        toast('✅ Data refreshed');
    } catch (err) { console.error(err); setApiStatus(false); toast('❌ Cannot reach server', 'error'); }
...
function startNotifPolling() { fetchNotifCounts(); notifInterval = setInterval(fetchNotifCounts, 10000); }
...
function renderProductsPanel() {
    ...
    <tr>
      ...
      <td><strong>${p.name}</strong>${p.image ? `<br><img src="${API_BASE.replace('/api', '')}/uploads/${p.image}" style="width:40px;height:40px;object-fit:cover;border-radius:6px;margin-top:4px;">` : ''}</td>
