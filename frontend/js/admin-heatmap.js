...
async function initOrderHeatmap() {
    ...
    // Load data
    try {
        var data = await apiFetch('/analytics/areas?period=30d');
        var areas = data.salesByLocation || [];
        
        renderHeatmapCircles(areas);
        renderHeatmapStats(areas);
    } catch (e) {
        console.error('Heatmap error:', e);
    }
}

function renderHeatmapCircles(areas) {
    ...
    areas.forEach(function(area) {
        ...
        // Color logic based on order volume
        var color = '#16a34a'; // Green (Low)
        if (area.orders >= 50) color = '#dc2626'; // Red (High)
        else if (area.orders >= 10) color = '#f59e0b'; // Amber (Medium)
        ...
        circle.bindPopup(
            ...
            '<div style="display:flex; justify-content:space-between; margin-bottom:4px;"><span>Orders:</span><strong>' + area.orders + '</strong></div>' +
            '<div style="display:flex; justify-content:space-between; margin-bottom:4px;"><span>Revenue:</span><strong>₹' + (area.revenue || 0).toLocaleString() + '</strong></div>' +
...
function renderHeatmapStats(areas) {
    ...
    // Sort by volume
    var sorted = [...areas].sort((a, b) => b.orders - a.orders);

    listEl.innerHTML = sorted.map(function(area) {
        var color = area.orders >= 50 ? 'var(--red)' : (area.orders >= 10 ? 'var(--amber)' : 'var(--green)');
        return '<div class="live-db-item" style="cursor:pointer;" onclick="focusHeatmapArea(' + area.lat + ',' + area.lng + ')">' +
            '<div class="live-db-avatar" style="background:' + color + '15; color:' + color + '; font-size:12px; font-weight:700;">' + area.orders + '</div>' +
            '<div class="live-db-info">' +
                '<div class="live-db-name">' + area.name + '</div>' +
                '<div class="live-db-meta">₹' + (area.revenue || 0).toLocaleString() + ' · ' + (area.customerCount || 0) + ' buyers</div>' +
            '</div>' +
